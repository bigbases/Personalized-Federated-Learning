...
class Server(object):
    def __init__(self, args, times):
        ...
        self.losses_by_clients = defaultdict(list)
        self.test_way = args.test_way
        ...
        self.rs_train_loss = []
        self.rs_proto_loss = []
        self.rs_proto_loss_body = []
        self.rs_proto_loss_per = []
        ...
        self.acc_by_clients = defaultdict(list)
    ...
    def save_global_model(self):
        ...
        model_path = os.path.join(model_path, self.data_dir.split('/')[-1] + "_" + self.algorithm + "_" + self.model_name + "_server" + ".pt")
    ...
    def save_results(self):
        ...
        if (len(self.rs_test_acc)):
            ...
            with h5py.File(file_path, 'w') as hf:
                ...
                hf.create_dataset('last_train_loss_by_clients', data=self.ltl)
                hf.create_dataset('last_test_acc_by_clients', data=self.lta)

            with open(result_path + f"{self.goal}_acc_{str(self.times)}.pkl", 'wb') as f:
    ...
    def check_params(self, flag):
        if flag:
            print('-'*20, 'For Global Evaluation', '-'*20)
            print(f'Client {self.selected_clients[0].id}: {self.selected_clients[0].model.head.weight.data[0, 0]}')
            print(f'Client {self.selected_clients[1].id}: {self.selected_clients[1].model.head.weight.data[0, 0]}')
        else:
            print('-'*20, 'For Personalized Evaluation', '-'*20)
            print(f'Client {self.selected_clients[0].id}: {self.selected_clients[0].model.head.weight.data[0, 0]}')
            print(f'Client {self.selected_clients[1].id}: {self.selected_clients[1].model.head.weight.data[0, 0]}')
    ...
    def test_metrics(self):
        ...
        for c in self.clients:
            ...
            self.acc_by_clients[c.id].append(ct*1.0 / ns)
    ...
    def evaluate(self, acc=None, loss=None):
        ...
        self.ltl = [l / n for l, n in zip(losses, num_samples_)]

        if self.test_way == "ind":
            test_acc = np.mean(accs)
            train_loss = np.mean(self.ltl)
